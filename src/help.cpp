...
//////////////////////////////////////////////////////////////
/////////////////// read retry finished with random data  and per page error analysis/////
	// let us start our operation in a test block
	// .. following is the address that we plan on using
	// .. the address of the block is 5-bytes hex number as specified in datasheet
	// .. the address is in the form of C1,C2 and R1,R2,R3
	uint8_t my_test_block_address[5] = {0x0,0x0,0x0,0x70,0x01};
	// let us try to analyze REF3 voltage
	// .. let us erase the block to all 1s
	// .. and program all MSB pages to all 0s
	// .. all LSB pages will be in 1s state
	...
	uint16_t msb_page_indices[num_msb_pages];
	uint16_t lsb_page_indices[num_msb_pages];
	lsb_page_indices[0] = 32;
	msb_page_indices[0] = 64;
	
	lsb_page_indices[1] = 49;
	msb_page_indices[1] = 98;

	// lsb_page_indices[2] = 69;
	// msb_page_indices[2] = 132;

	uint16_t idx_pointer = 2;
	for(uint16_t l_idx = 69;(l_idx<960&&idx_pointer<32);l_idx+=66,idx_pointer++)
	{
		lsb_page_indices[idx_pointer] = l_idx;
		msb_page_indices[idx_pointer] = l_idx+63;
		cout<<"LSB and MSB Pages are: "<<lsb_page_indices[idx_pointer]<<","<<msb_page_indices[idx_pointer]<<endl;
	}

	lsb_page_indices[idx_pointer] = 931;
	msb_page_indices[idx_pointer] = 931+63;

	num_msb_pages = idx_pointer+1;
	// num_msb_pages = 1;
	uint16_t both_lsb_msb_indices[num_msb_pages*2];
	for(uint16_t b_idx = 0;b_idx<num_msb_pages;b_idx++)
	{
	...
	cout<<"Populated "<<num_msb_pages<<" LSB and MSB pages indices"<<endl;
	// if(msb_page_indices[idx_pointer-1] != 1022 || idx_pointer!=num_msb_pages)
	// {
	// 	cout<<"Error in assigning the msb and lsb indices"<<endl;
	// 	cout<<"\t.. the last msb index should be 1022, found "<<msb_page_indices[idx_pointer-1]<<endl;
	...
	std::fstream read_retry_file;
	read_retry_file.open("read_retry_file.txt",std::fstream::out);
	std::fstream page_raw_data_file;
	page_raw_data_file.open("page_raw_data_file.txt",std::fstream::out);
	std::fstream per_page_error_file;
	per_page_error_file.open("per_page_error_file.txt",std::fstream::out);
	std::fstream per_page_classification_file;
	per_page_classification_file.open("per_page_classification_file.txt",std::fstream::out);

	// convert to SLC for SLC wala
	// onfi_instance.convert_to_slc(my_test_block_address, true);
	
	// create random data to program into each page
	bool program_provided_data = false;
	// program msb pages
	bool program_msb = true;
	// program lsb pages
	bool program_lsb = true;
	// if no program at all, set following to true
	...
	{
		// the value will be between 1 and 255
		provided_data_msb[data_byte_id] = 0xff;//(uint8_t)(rand()%255 +1);
		provided_data_lsb[data_byte_id] = 0x00;//(uint8_t)(rand()%255 +1);
	}
	...
	if(program_provided_data)
	{
		...
		{
			// memset lsb so that they are also programmed
			memset(provided_data_lsb,0xff,(onfi_instance.num_bytes_in_page+onfi_instance.num_spare_bytes_in_page));
			onfi_instance.program_pages_in_a_block_data(my_test_block_address, provided_data_lsb, false,lsb_page_indices,num_msb_pages);
			onfi_instance.program_pages_in_a_block_data(my_test_block_address, provided_data_msb, false,msb_page_indices,num_msb_pages);
		}
		else if(program_lsb) 
		{
			...
			// memset msb so that they are also programmed
			memset(provided_data_msb,0xff,(onfi_instance.num_bytes_in_page+onfi_instance.num_spare_bytes_in_page));
			onfi_instance.program_pages_in_a_block_data(my_test_block_address, provided_data_msb, false,msb_page_indices,num_msb_pages);
		}else if(erase_only)
		{
	...
	else
	{
		...
		else if(program_msb)
		{
			...
			onfi_instance.program_pages_in_a_block(my_test_block_address,false,msb_page_indices,num_msb_pages);
		}
		else if(program_lsb)
		{
			onfi_instance.program_pages_in_a_block(my_test_block_address,false,lsb_page_indices,num_msb_pages);
	...
	per_page_error_file<<"read_retry_level"<<", lsb_page_indices"<<", msb_page_indices"<<", "<<"msb_0 (prog)"<<", "<<"msb_0 (read)"<<", "<<"msb_1 (prog)"<<", "<<"msb_1 (read)"<<", "<<"lsb_0 (prog)"<<", "<<"lsb_0 (read)"<<", "<<"lsb_1 (prog)"<<", "<<"lsb_1 (read)"<<", "<<"bit_state_11 (prog)"<<", "<<"bit_state_11 (read)"<<", "<<"bit_state_01 (prog)"<<", "<<"bit_state_01 (read)"<<", "<<"bit_state_00 (prog)"<<", "<<"bit_state_00 (read)"<<", "<<"bit_state_10 (prog)"<<", "<<"bit_state_10 (read)"<<endl;
	per_page_classification_file<<"read_retry_level"<<", lsb_page_indices"<<", msb_page_indices"<<endl;

	for(uint8_t read_retry_level=0;read_retry_level<24;read_retry_level++)
	{
		//  out counter for stats
		...
		{
			//  out counter for stats
			// these are temp stats just for this page
			unsigned int msb_0_read_page = 0,msb_1_read_page = 0,lsb_0_read_page = 0, lsb_1_read_page = 0, bit_state_00_read_page = 0, bit_state_10_read_page = 0, bit_state_01_read_page = 0, bit_state_11_read_page = 0;
			unsigned int msb_0_prog_page = 0,msb_1_prog_page = 0,lsb_0_prog_page = 0, lsb_1_prog_page = 0, bit_state_00_prog_page = 0, bit_state_10_prog_page = 0, bit_state_01_prog_page = 0, bit_state_11_prog_page = 0;
			
			unordered_map<string,int> state_mismatch_map_page;
			state_mismatch_map_page["00_00"] = 0;
			state_mismatch_map_page["00_01"] = 0;
			state_mismatch_map_page["00_11"] = 0;
			state_mismatch_map_page["00_10"] = 0;

			state_mismatch_map_page["01_00"] = 0;
			state_mismatch_map_page["01_01"] = 0;
			state_mismatch_map_page["01_11"] = 0;
			state_mismatch_map_page["01_10"] = 0;

			state_mismatch_map_page["10_00"] = 0;
			state_mismatch_map_page["10_01"] = 0;
			state_mismatch_map_page["10_11"] = 0;
			state_mismatch_map_page["10_10"] = 0;

			state_mismatch_map_page["11_00"] = 0;
			state_mismatch_map_page["11_01"] = 0;
			state_mismatch_map_page["11_11"] = 0;
			state_mismatch_map_page["11_10"] = 0;

			unsigned int error_bit_num_page = 0;

			// read this page right now
			onfi_instance.read_page_and_return_value(my_test_block_address,lsb_page_indices[page1_idx],page_data_read_lsb,true);
			page_raw_data_file<<"LSB";
			for(uint16_t byte_idx = 0;byte_idx<onfi_instance.num_bytes_in_page+onfi_instance.num_spare_bytes_in_page;byte_idx++)
			{
				page_raw_data_file<<page_data_read_lsb[byte_idx];
			}

			// read the msb page right now
			onfi_instance.read_page_and_return_value(my_test_block_address,msb_page_indices[page1_idx],page_data_read_msb,true);
			page_raw_data_file<<"MSB";
			for(uint16_t byte_idx = 0;byte_idx<onfi_instance.num_bytes_in_page+onfi_instance.num_spare_bytes_in_page;byte_idx++)
			{
				page_raw_data_file<<page_data_read_msb[byte_idx];
			}
			...
			for(uint16_t byte_idx = 0;byte_idx<onfi_instance.num_bytes_in_page+onfi_instance.num_spare_bytes_in_page;byte_idx++)
			{
				...
				}else
				{
					...
					lsb_1_read_page += num_ones;
					lsb_0_read_page += 8-num_ones;
				}
				...
				}else
				{
					...
					msb_1_read_page += num_ones;
					msb_0_read_page += 8-num_ones;
				}
				...
				}else
				{
					...
					lsb_1_prog_page += num_ones;
					lsb_0_prog_page += 8-num_ones;
				}
				...
				}else
				{
					...
					msb_1_prog_page += num_ones;
					msb_0_prog_page += 8-num_ones;
				}

				// compute BER here
				error_bit_num += (find_num_1s(prog_msb_byte^read_msb_byte)+find_num_1s(prog_lsb_byte^read_lsb_byte));
				error_bit_num_page += (find_num_1s(prog_msb_byte^read_msb_byte)+find_num_1s(prog_lsb_byte^read_lsb_byte));
				...
				for(uint8_t bit_idx=0;bit_idx<8;bit_idx++)
				{
					...
					state_mismatch_map_page[prog_state+"_"+read_state]++;
				}
			}

			// write data here that will be for each pages
			per_page_error_file<<(int)read_retry_level<<","<<lsb_page_indices[page1_idx]<<","<<msb_page_indices[page1_idx]<<", "<<msb_0_prog<<", "<<msb_0_read<<", "<<msb_1_prog<<", "<<msb_1_read<<", "<<lsb_0_prog<<", "<<lsb_0_read<<", "<<lsb_1_prog<<", "<<lsb_1_read<<", "<<bit_state_11_prog<<", "<<bit_state_11_read<<", "<<bit_state_01_prog<<", "<<bit_state_01_read<<", "<<bit_state_00_prog<<", "<<bit_state_00_read<<", "<<bit_state_10_prog<<", "<<bit_state_10_read;
			// iterate through all the items in the map and print them in the same line
			for(auto my_iter: state_mismatch_map_page)
			{
				per_page_error_file<<", "<<my_iter.first<<", "<<my_iter.second;
			}
			per_page_error_file<<", E:"<<error_bit_num_page<<endl;

			// just the classification part in a file
			per_page_classification_file<<(int)read_retry_level<<","<<lsb_page_indices[page1_idx]<<","<<msb_page_indices[page1_idx];
			for(auto my_iter: state_mismatch_map_page)
			{
				per_page_classification_file<<", "<<my_iter.first<<", "<<my_iter.second;
			}
			per_page_classification_file<<", E:"<<error_bit_num_page<<endl;
		}
	...
	per_page_error_file.close();
	page_raw_data_file.close();
	read_retry_file.close();
	per_page_classification_file.close();
