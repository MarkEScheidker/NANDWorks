...
__attribute__((always_inline)) void interface::set_pin_direction_inactive()
{
    ...
    gpioSetMode(WP_PIN, PI_OUTPUT); gpioWrite(WP_PIN, 1); // Active low
    gpioSetMode(CLE_PIN, PI_OUTPUT); gpioWrite(CLE_PIN, 0); // Active high
    gpioSetMode(ALE_PIN, PI_OUTPUT); gpioWrite(ALE_PIN, 0); // Active high
    gpioSetMode(RE_PIN, PI_OUTPUT); gpioWrite(RE_PIN, 1); // Active low
    gpioSetMode(WE_PIN, PI_OUTPUT); gpioWrite(WE_PIN, 1); // Active low
    gpioSetMode(CE_PIN, PI_OUTPUT); gpioWrite(CE_PIN, 1); // Active low
    ...
    gpioSetMode(DQ1_PIN, PI_OUTPUT); gpioWrite(DQ1_PIN, 0);
    gpioSetMode(DQ2_PIN, PI_OUTPUT); gpioWrite(DQ2_PIN, 0);
    gpioSetMode(DQ3_PIN, PI_OUTPUT); gpioWrite(DQ3_PIN, 0);
    gpioSetMode(DQ4_PIN, PI_OUTPUT); gpioWrite(DQ4_PIN, 0);
    gpioSetMode(DQ5_PIN, PI_OUTPUT); gpioWrite(DQ5_PIN, 0);
    gpioSetMode(DQ6_PIN, PI_OUTPUT); gpioWrite(DQ6_PIN, 0);
    gpioSetMode(DQ7_PIN, PI_OUTPUT); gpioWrite(DQ7_PIN, 0);

    // Set DQS and DQSc to output and low
    gpioSetMode(DQS_PIN, PI_OUTPUT); gpioWrite(DQS_PIN, 0);
    gpioSetMode(DQSc_PIN, PI_OUTPUT); gpioWrite(DQSc_PIN, 0);
}
