...
__attribute__((always_inline)) void interface::set_pin_direction_inactive()
{
    ...
    gpioSetMode(GPIO_WP, PI_OUTPUT); gpioWrite(GPIO_WP, 1); // Active low
    gpioSetMode(GPIO_CLE, PI_OUTPUT); gpioWrite(GPIO_CLE, 0); // Active high
    gpioSetMode(GPIO_ALE, PI_OUTPUT); gpioWrite(GPIO_ALE, 0); // Active high
    gpioSetMode(GPIO_RE, PI_OUTPUT); gpioWrite(GPIO_RE, 1); // Active low
    gpioSetMode(GPIO_WE, PI_OUTPUT); gpioWrite(GPIO_WE, 1); // Active low
    gpioSetMode(GPIO_CE, PI_OUTPUT); gpioWrite(GPIO_CE, 1); // Active low
    ...
    gpioSetMode(GPIO_DQ0, PI_OUTPUT); gpioWrite(GPIO_DQ0, 0);
    gpioSetMode(GPIO_DQ1, PI_OUTPUT); gpioWrite(GPIO_DQ1, 0);
    gpioSetMode(GPIO_DQ2, PI_OUTPUT); gpioWrite(GPIO_DQ2, 0);
    gpioSetMode(GPIO_DQ3, PI_OUTPUT); gpioWrite(GPIO_DQ3, 0);
    gpioSetMode(GPIO_DQ4, PI_OUTPUT); gpioWrite(GPIO_DQ4, 0);
    gpioSetMode(GPIO_DQ5, PI_OUTPUT); gpioWrite(GPIO_DQ5, 0);
    gpioSetMode(GPIO_DQ6, PI_OUTPUT); gpioWrite(GPIO_DQ6, 0);
    gpioSetMode(GPIO_DQ7, PI_OUTPUT); gpioWrite(GPIO_DQ7, 0);

    // Set DQS and DQSc to output and low
    gpioSetMode(GPIO_DQS, PI_OUTPUT); gpioWrite(GPIO_DQS, 0);
    gpioSetMode(GPIO_DQSC, PI_OUTPUT); gpioWrite(GPIO_DQSC, 0);
}
